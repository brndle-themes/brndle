...
<section class="py-24 md:py-32 {{ ($a['variant'] ?? 'light') === 'dark' ? 'bg-surface-inverse text-white' : 'bg-white' }}">
  <div class="max-w-7xl mx-auto px-6">
    @if($a['title'])
      <div class="max-w-3xl mx-auto text-center mb-20 reveal">
        ...
        @if($a['subtitle'])
          <p class="mt-4 text-lg {{ ($a['variant'] ?? 'light') === 'dark' ? 'text-slate-400' : 'text-text-secondary' }}">{{ $a['subtitle'] }}</p>
        @endif
      </div>
    @endif
      ...
      @foreach(($a['features'] ?? []) as $i => $feature)
        <div class="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center reveal {{ $i % 2 !== 0 ? 'lg:[direction:rtl] lg:[&>*]:[direction:ltr]' : '' }}">
          <div>
            ...
            <p class="mt-4 text-lg {{ ($a['variant'] ?? 'light') === 'dark' ? 'text-slate-400' : 'text-text-secondary' }} leading-relaxed">{{ $feature['description'] ?? '' }}</p>
              ...
              <ul class="mt-6 space-y-3">
                @foreach($feature['bullets'] as $bullet)
                  <li class="flex items-start gap-3">
                    ...
                    <span class="{{ ($a['variant'] ?? 'light') === 'dark' ? 'text-slate-300' : 'text-text-secondary' }}">{{ $bullet }}</span>
                  </li>
                @endforeach
              </ul>
            @endif
          ...
          @if(isset($feature['image']))
            <div class="rounded-2xl {{ ($a['variant'] ?? 'light') === 'dark' ? 'border border-white/10' : 'bg-surface-secondary border border-slate-200 shadow-lg' }} overflow-hidden">
