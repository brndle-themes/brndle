...
<section class="py-24 md:py-32 {{ ($a['variant'] ?? 'light') === 'dark' ? 'bg-[var(--color-surface-inverse)] text-white' : 'bg-[var(--color-surface-primary)]' }}">
  <div class="max-w-7xl mx-auto px-6">
    @if($a['title'])
      <div class="max-w-3xl mx-auto text-center mb-20 reveal">
        ...
        @if($a['subtitle'])
          <p class="mt-4 text-lg {{ ($a['variant'] ?? 'light') === 'dark' ? 'text-[var(--color-text-secondary)]' : 'text-[var(--color-text-secondary)]' }}">{{ $a['subtitle'] }}</p>
        @endif
      </div>
    @endif
        ...
        <div class="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center reveal {{ $i % 2 !== 0 ? 'lg:[direction:rtl] lg:[&>*]:[direction:ltr]' : '' }}">
          <div>
            @if(isset($feature['icon']))
              <div class="w-12 h-12 rounded-xl bg-[var(--color-accent-subtle)] flex items-center justify-center mb-6">
                <span class="text-[var(--color-accent)] text-2xl">{!! $feature['icon'] !!}</span>
              </div>
            @endif
            <h3 class="text-2xl sm:text-3xl font-bold tracking-tight">{{ $feature['title'] ?? '' }}</h3>
            <p class="mt-4 text-lg {{ ($a['variant'] ?? 'light') === 'dark' ? 'text-[var(--color-text-secondary)]' : 'text-[var(--color-text-secondary)]' }} leading-relaxed">{{ $feature['description'] ?? '' }}</p>
              ...
              <ul class="mt-6 space-y-3">
                @foreach($feature['bullets'] as $bullet)
                  <li class="flex items-start gap-3">
                    ...
                    <span class="{{ ($a['variant'] ?? 'light') === 'dark' ? 'text-[var(--color-text-secondary)]' : 'text-[var(--color-text-secondary)]' }}">{{ $bullet }}</span>
                  </li>
                @endforeach
              </ul>
            @endif
          ...
          @if(isset($feature['image']))
            <div class="rounded-2xl {{ ($a['variant'] ?? 'light') === 'dark' ? 'border border-white/10' : 'bg-[var(--color-surface-secondary)] border border-[var(--color-surface-tertiary)] shadow-lg' }} overflow-hidden">
