...
<section class="relative py-24 md:py-32 overflow-hidden {{ ($a['variant'] ?? 'dark') === 'dark' ? 'bg-surface-inverse' : 'bg-accent' }} noise">
  <div class="absolute inset-0">
    ...
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[40rem] h-[40rem] rounded-full bg-indigo-600/10 blur-[128px]"></div>
  </div>
  ...
  <div class="relative z-10 max-w-3xl mx-auto px-6 text-center reveal">
    ...
    @if($a['subtitle'])
      <p class="mt-5 text-lg text-slate-400 max-w-xl mx-auto">{{ $a['subtitle'] }}</p>
    @endif
    ...
    <div class="mt-10 flex flex-wrap justify-center gap-4">
      ...
      @if($a['cta_secondary'])
        <a href="{{ esc_url($a['cta_secondary_url']) }}" class="inline-flex items-center gap-2 px-8 py-4 text-[0.95rem] font-medium rounded-xl border border-white/10 text-slate-300 hover:bg-white/5 hover:border-white/20 transition-all duration-300">
          {{ $a['cta_secondary'] }}
        </a>
      @endif
    </div>
