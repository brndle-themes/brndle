@php
  ...
  $isDarkFooter = $style === 'dark';
  $links = is_callable($socialLinks) ? $socialLinks() : (is_array($socialLinks) ? $socialLinks : []);
@endphp

<footer class="{{ $isDarkFooter ? 'bg-[#080B16] text-white' : 'bg-[var(--color-surface-secondary)] text-[var(--color-text-primary)]' }}">
  <div class="max-w-7xl mx-auto px-6 py-12">
    <div class="flex flex-col sm:flex-row items-center justify-between gap-6">
      ...
      {{-- Copyright --}}
      <p class="text-sm {{ $isDarkFooter ? 'text-slate-500' : 'text-[var(--color-text-tertiary)]' }}">
        {!! $footerCopyright !!}
      </p>
        ...
        <div class="flex items-center gap-5">
          @if(!empty($links['twitter']))
            <a href="{{ esc_url($links['twitter']) }}" target="_blank" rel="noopener noreferrer"
               class="{{ $isDarkFooter ? 'text-slate-500 hover:text-white' : 'text-[var(--color-text-tertiary)] hover:text-[var(--color-text-primary)]' }} transition-colors"
