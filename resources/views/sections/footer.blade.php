...
<footer class="{{ $isDarkFooter ? 'bg-[#080B16] text-white' : 'bg-surface-secondary text-text-primary' }}">
  <div class="max-w-7xl mx-auto px-6 py-12">
    <div class="flex flex-col sm:flex-row items-center justify-between gap-6">
      ...
      {{-- Copyright --}}
      <p class="text-sm {{ $isDarkFooter ? 'text-slate-500' : 'text-text-tertiary' }}">
        {!! $footerCopyright !!}
      </p>
        ...
        <div class="flex items-center gap-5">
          @if(!empty($links['twitter']))
            <a href="{{ esc_url($links['twitter']) }}" target="_blank" rel="noopener noreferrer"
               class="{{ $isDarkFooter ? 'text-slate-500 hover:text-white' : 'text-text-tertiary hover:text-text-primary' }} transition-colors"
