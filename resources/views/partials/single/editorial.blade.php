...
<article @php(post_class('h-entry'))>
  ...
  <header class="max-w-4xl mx-auto px-6 pt-16 pb-10">
    ...
    <div class="flex flex-wrap items-center gap-3 text-sm text-[var(--color-text-tertiary)]">
      @if($category = get_the_category())
        <a
          href="{{ get_category_link($category[0]->term_id) }}"
          class="font-semibold uppercase tracking-wider text-[var(--color-accent)] hover:opacity-80 transition-opacity text-xs"
        >
          {{ $category[0]->name }}
        </a>
        <span>&middot;</span>
    ...
    <h1 class="mt-6 text-4xl sm:text-5xl lg:text-6xl font-heading font-bold tracking-tight leading-[1.1] text-[var(--color-text-primary)] [text-wrap:balance] p-name">
      {!! $title !!}
    </h1>

    {{-- Meta --}}
    <div class="mt-8 flex flex-wrap items-center gap-4 text-sm text-[var(--color-text-tertiary)]">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          @if($singleShowReadingTime)
            <span class="text-xs text-[var(--color-text-tertiary)]">{{ $readingTime }}</span>
          @endif
        </div>
      </div>
    </div>
    ...
    {{-- Divider --}}
    <div class="mt-8 border-t border-[var(--color-surface-tertiary)]"></div>
  </header>
    ...
    <div class="max-w-5xl mx-auto px-6 mb-14">
      <figure>
        ...
        @if($caption = get_the_post_thumbnail_caption())
          <figcaption class="mt-3 text-center text-sm text-[var(--color-text-tertiary)] italic">
            {{ $caption }}
          </figcaption>
        @endif
      </figure>
  ...
  <div class="max-w-[700px] mx-auto px-6 e-content">
    <div class="editorial-content prose prose-lg max-w-none leading-[1.8]
      ...
      prose-blockquote:pl-6 prose-blockquote:text-[var(--color-text-secondary)]
      ...
      first-of-type:prose-p:first-letter:text-[var(--color-accent)]">
      @php(the_content())
    </div>
  </div>
