    ...
    <div class="space-y-4 reveal">
      @foreach(($a['items'] ?? []) as $i => $item)
        <details class="group rounded-2xl border {{ $isDark ? 'border-white/[0.08] bg-white/[0.02]' : 'border-surface-tertiary bg-surface-primary' }} overflow-hidden hover:border-text-tertiary transition-colors" id="faq-{{ $i }}">
          <summary class="flex items-center justify-between cursor-pointer px-6 py-5 text-left focus:outline-2 focus:outline-offset-2 focus:outline-accent" aria-controls="faq-answer-{{ $i }}">
          ...
          <div class="px-6 pb-5 {{ $isDark ? 'text-white/70' : 'text-text-secondary' }} leading-relaxed" id="faq-answer-{{ $i }}">
