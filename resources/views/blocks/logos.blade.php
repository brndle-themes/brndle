    ...
    <div class="flex flex-wrap items-center justify-center gap-x-8 gap-y-6">
      @foreach(($a['companies'] ?? []) as $company)
        @if(is_array($company) && isset($company['url']))
          <x-img :src="$company['url']" :alt="$company['alt'] ?? ($company['name'] ?? '')" class="h-[52px] w-auto {{ ($a['variant'] ?? 'light') === 'dark' ? 'grayscale invert opacity-60 hover:grayscale-0 hover:invert-0' : 'opacity-80 hover:opacity-100' }} transition-opacity duration-300 motion-reduce:transition-none" />
