      ...
      <div class="max-w-2xl mx-auto">
        @foreach($steps as $i => $step)
          <div class="relative flex gap-6 {{ $i < count($steps) - 1 ? 'pb-12' : '' }} reveal">
            ...
            @endif
            <div class="relative z-10 flex-shrink-0 w-10 h-10 rounded-full brndle-section-accent bg-accent text-on-accent flex items-center justify-center text-sm font-bold">
              @if(!empty($step['icon']))
                {{ $step['icon'] }}
              @else
                {{ $i + 1 }}
      ...
      <div class="grid {{ $stepCols[max(min(count($steps), 4) - 1, 0)] ?? 'md:grid-cols-3' }} gap-8">
        @foreach($steps as $i => $step)
          <div class="relative text-center reveal">
            ...
            @endif
            <div class="relative z-10 w-12 h-12 rounded-full brndle-section-accent bg-accent text-on-accent flex items-center justify-center text-lg font-bold mx-auto mb-4">
              @if(!empty($step['icon']))
                {{ $step['icon'] }}
              @else
                {{ $i + 1 }}
