...
<section class="py-24 md:py-32 {{ ($a['variant'] ?? 'light') === 'dark' ? 'brndle-section-dark' : 'bg-surface-secondary' }}">
  <div class="max-w-7xl mx-auto px-6">
    ...
    @php($plans = $a['plans'] ?? [])
    @php($gridCols = ['md:grid-cols-1', 'md:grid-cols-2', 'md:grid-cols-3'])
    <div class="grid {{ $gridCols[min(count($plans), 3) - 1] ?? 'md:grid-cols-3' }} gap-6 max-w-5xl mx-auto">
      @foreach($plans as $plan)
        @php($featured = $plan['featured'] ?? false)
        <div class="p-8 rounded-2xl reveal transition-all duration-300 hover:shadow-lg {{ $featured ? 'brndle-section-dark border-2 border-accent relative' : 'bg-surface-primary border border-surface-tertiary hover:border-text-tertiary' }}" {!! $featured ? 'aria-label="' . esc_attr($plan['badge'] ?? __('Most Popular', 'brndle')) . ' plan"' : '' !!}>
          @if($featured)
            <div class="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-accent text-white text-xs font-bold" aria-hidden="true">{{ $plan['badge'] ?? __('Most Popular', 'brndle') }}</div>
          @endif
