  ...
  <div class="max-w-7xl mx-auto px-6">
    ...
    <div class="grid md:grid-cols-{{ min(count($plans), 3) }} gap-6 max-w-5xl mx-auto">
      @foreach($plans as $plan)
        ...
        <div class="p-8 rounded-2xl reveal transition-all duration-300 hover:shadow-lg {{ $featured ? 'bg-surface-inverse text-white border-2 border-accent relative' : 'bg-surface-primary border border-surface-tertiary hover:border-text-tertiary' }}">
          @if($featured)
          ...
          @if(isset($plan['description']))
            <p class="text-sm {{ $featured ? 'text-text-secondary' : 'text-text-secondary' }} mt-1">{{ $plan['description'] }}</p>
          @endif

          <div class="mt-6 mb-8">
            <span class="text-5xl font-bold">{{ $plan['price'] ?? '' }}</span>
            @if(isset($plan['period']))
              <span class="{{ $featured ? 'text-text-secondary' : 'text-text-tertiary' }} ml-1">{{ $plan['period'] }}</span>
            @endif
          </div>
          ...
          @if(isset($plan['features']) && is_array($plan['features']))
            <ul class="space-y-3 mb-8">
              @foreach($plan['features'] as $feature)
                <li class="flex items-center gap-3 text-sm {{ $featured ? 'text-text-secondary' : 'text-text-secondary' }}">
          ...
          <a href="{{ esc_url($plan['cta_url'] ?? '#') }}" class="block w-full text-center px-6 py-3 text-sm font-semibold rounded-xl transition-all {{ $featured ? 'bg-accent hover:opacity-90 text-white hover:-translate-y-px hover:shadow-lg hover:shadow-[var(--color-accent)]/25' : 'border border-surface-tertiary text-text-secondary hover:bg-surface-secondary hover:border-text-tertiary' }}">
