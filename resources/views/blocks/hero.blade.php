...
<section class="relative min-h-screen flex items-center overflow-hidden {{ $isDark ? 'bg-surface-inverse text-white' : 'bg-surface-primary text-text-primary' }} noise">
  {{-- Ambient --}}
  ...
  <div class="relative z-10 w-full max-w-7xl mx-auto px-6 pt-32 pb-20">
    <div class="max-w-4xl">
      @if($a['eyebrow'])
        <div class="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full border border-white/[0.08] bg-white/[0.03] backdrop-blur-sm mb-8">
          ...
          <span class="text-sm font-medium {{ $isDark ? 'text-text-secondary' : 'text-text-secondary' }}">{{ $a['eyebrow'] }}</span>
        </div>
      @endif
      ...
      @if($a['subtitle'])
        <p class="mt-6 text-[clamp(1.05rem,1.8vw,1.25rem)] leading-relaxed {{ $isDark ? 'text-text-secondary' : 'text-text-secondary' }} max-w-2xl">
          {{ $a['subtitle'] }}
        </p>
      @endif
      ...
      <div class="mt-10 flex flex-wrap items-center gap-4">
        @if($a['cta_primary'])
          <a href="{{ esc_url($a['cta_primary_url']) }}" class="group inline-flex items-center gap-2 px-7 py-3.5 text-[0.925rem] font-semibold rounded-xl {{ $isDark ? 'bg-white text-slate-900 hover:shadow-[0_0_40px_rgba(255,255,255,0.12)]' : 'bg-surface-inverse text-white hover:opacity-90' }} transition-all duration-300 hover:-translate-y-0.5">
            {{ $a['cta_primary'] }}
        ...
        @if($a['cta_secondary'])
          <a href="{{ esc_url($a['cta_secondary_url']) }}" class="inline-flex items-center gap-2 px-7 py-3.5 text-[0.925rem] font-medium rounded-xl border {{ $isDark ? 'border-white/[0.1] text-text-secondary hover:bg-white/[0.04]' : 'border-surface-tertiary text-text-secondary hover:bg-surface-secondary' }} transition-all duration-300">
            {{ $a['cta_secondary'] }}
          </a>
        @endif
      </div>
    ...
    @if(!empty($a['logos']))
      <div class="mt-20 pt-10 border-t {{ $isDark ? 'border-white/[0.06]' : 'border-surface-tertiary' }}">
        <p class="text-[11px] font-semibold uppercase tracking-[0.2em] {{ $isDark ? 'text-text-tertiary' : 'text-text-tertiary' }} mb-8">{{ __('Trusted by industry leaders', 'brndle') }}</p>
        <div class="flex flex-wrap items-center gap-x-12 gap-y-6">
          @foreach($a['logos'] as $logo)
            ...
            @elseif(is_string($logo))
              <span class="text-lg font-bold {{ $isDark ? 'text-text-tertiary' : 'text-text-tertiary' }} tracking-tight">{{ $logo }}</span>
            @endif
          @endforeach
        </div>
      </div>
