...
<section class="relative min-h-screen flex items-center overflow-hidden {{ $isDark ? 'brndle-section-dark' : 'bg-surface-primary text-text-primary' }} noise">
  ...
  <div class="relative z-10 w-full max-w-7xl mx-auto px-6 pt-32 pb-20">
    <div class="max-w-4xl">
      ...
      @if($a['subtitle'])
        <p class="mt-6 text-[clamp(1.05rem,1.8vw,1.25rem)] leading-relaxed {{ $isDark ? 'text-white/70' : 'text-text-secondary' }} max-w-2xl">
          {{ $a['subtitle'] }}
        </p>
      @endif
    ...
    @if($a['image'])
      <div class="mt-20 relative">
        <div class="absolute -inset-4 bg-gradient-to-r from-accent/20 via-accent/10 to-cyan-500/20 rounded-2xl blur-2xl opacity-60"></div>
    ...
    @if(!empty($a['logos']))
      <div class="mt-20 pt-10 border-t {{ $isDark ? 'border-white/[0.06]' : 'border-surface-tertiary' }}">
        <p class="text-[11px] font-semibold uppercase tracking-[0.2em] {{ $isDark ? 'text-white/50' : 'text-text-tertiary' }} mb-8">{{ __('Trusted by industry leaders', 'brndle') }}</p>
